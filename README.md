...
| 解像度 | **1920×480**(9.16")。ハンドシェイク応答の自己申告幅から自動検出 — 6.86" 版(1280×480 / PID `0x5302`)は実験的サポート |
...
バックエンド(`server.py` / `server.exe`)と Electron シェルの挙動を上書きできます。
通常利用では設定不要です。

| 変数 | 既定値 | 用途 |
|------|--------|------|
| `TROFEO_HOST` | `127.0.0.1` | WebSocket のバインド先。`0.0.0.0` で LAN に公開(自己責任) |
| `TROFEO_PORT` | `8787` | WebSocket ポート。開発版を常駐版と並走させるときに変更 |
| `TROFEO_WIDTH` / `TROFEO_HEIGHT` | 自動検出 | パネル解像度の強制上書き(自己申告が壊れている個体向け) |
| `TROFEO_VID` / `TROFEO_PID` | `0x0416` / `0x5408,0x5302` | USB ID の上書き(未知の LY パネルを試すとき) |
| `TROFEO_LOGLEVEL` | `info` | `debug` でポーリング単位の詳細を `backend.log` に出力 |
| `TROFEO_BACKEND_URL` | `ws://localhost:8787` | (Electron)レンダラーが接続するバックエンド URL |
| `TROFEO_DEV` | 未設定 | `1` でパッケージ版でも `DEBUG_*` フックを有効化 |
