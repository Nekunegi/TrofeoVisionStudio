...
export function ToastCard({ t, index, bg, panelW }: { t: LcdToast; index: number; bg: BgEnv; panelW: number }) {
  // Portrait (480w) can't fit the 540px design width — cap to panel with a
  // 20px gutter each side so every glyph stays inside the visible strip.
  const TW = Math.min(TOAST_W_MAX, panelW - 40)
